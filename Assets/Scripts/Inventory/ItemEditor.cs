...
public class ItemEditor : Editor
{
	...
	private void DrawIcon(Item item)
	{
		...
		using(SerializedProperty prop = serializedObject.FindProperty("_icon"))
		{
			...
			Sprite iconSprite = EditorGUILayout.ObjectField(item.Icon, typeof(Sprite), false) as Sprite;
	...
	private void DrawCrosshair(Item item)
	{
		...
		if((item.CrosshairType != null) && (item.CrosshairType.Sprite != null))
		{
			GUILayout.Box(item.CrosshairType.Sprite.texture, GUILayout.Width((64)), GUILayout.Height(64));
		}
		else
		{
		...
		{
			CrosshairType crosshairSprite = EditorGUILayout.ObjectField(item.CrosshairType, typeof(CrosshairType), false) as CrosshairType;
	...
	private void DrawAddAllAvailableUseActionsBtn(Item item, SerializedProperty useActionsProp)
	{
